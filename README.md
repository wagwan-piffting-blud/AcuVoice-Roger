...
Project spearheaded by Wags (https://wagspuzzle.space/), with _immense_ help from Claude Code (https://claude.ai/) for the SAPI shim, WASM build, reverse engineering, and overall project structure.

## WASM Build/Port

As of 2026-06-28, there is, thanks to Claude Code, a WebAssembly build of the AcuVoice Roger voice that can be used purely in web browsers/on phones/etc. See the `wasm/` folder for details. TL;DR: You can now run the Roger voice in a browser without installing anything (it runs the main synthesis DLL via a x86 emulator). The WASM build is also fully open source and equally licensed under GNU GPL v3.0.
